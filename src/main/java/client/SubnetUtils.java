...
public class SubnetUtils {
    ...
    private int matchAddress(Matcher matcher) {
        ...
        for(int i = 1; i <= 4; ++i) {
            int n = this.rangeCheck(Integer.parseInt(matcher.group(i)), -1, 255);
            addr |= (n & 255) << 8 * (4 - i);
        }
    ...
    public final class SubnetInfo {
        ...
        public int getAddressCount() {
            int count = this.broadcast() - this.network() + (SubnetUtils.this.isInclusiveHostCount() ? 1 : -1);
